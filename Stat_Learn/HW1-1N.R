...
gpredict=function(dftrain=NULL,dftest=NULL)
{
	...
	ret=list(mua=0, mub=rep(0,nfeature),s_ab = matrix(0, nrow=1, ncol=nfeature),s_bb=matrix(0, nrow=nfeature, ncol=nfeature),predict=c())
	x<-c(dftrain[,1])
	ret$mua=mean(x)
	...
	else
	{
		...
		a3= a1%*%t(a2)
		for(i in 1:ntest){
			ret$predict<-append(ret$predict,(ret$mua+a3[i]))
		}
		return(ret)
	}	
}
